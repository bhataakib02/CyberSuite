...
export default function MedicalRecordsPage() {
  ...
  const fetchMyRecords = async () => {
    ...
    try {
      ...
      if (res.ok) setMyRecords(data.records || []);
    } catch (err) {
      console.error(err);
    } finally {
      setIsLoading(false);
  ...
  const fetchSharedRecords = async () => {
    try {
      ...
      const data = await res.json();
      if (res.ok) setSharedRecords(data.accesses || []);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    data.append('encKeyForOwner', 'SIMULATED_ENC_KEY_' + Date.now());
    ...
    try {
      const res = await apiFetch('/medical/grant-access', {
        ...
        body: JSON.stringify({
          ...
          expiresInHours,
          encKey: 'SIMULATED_KEY_EXCHANGE_FOR_DOCTOR'
        }),
      });
            ...
            >
              ...
              <form onSubmit={handleUpload} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <input type="text" value={description} onChange={e => setDescription(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-2xl px-5 py-4 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/30 transition-all font-medium" placeholder="E.g. MRI Scan Results Q1" />
                </div>
                
                <div className="flex gap-4 pt-6">
                  <button type="button" onClick={() => setIsUploadModalOpen(false)} className="flex-1 py-4 rounded-2xl text-xs font-black uppercase tracking-widest text-zinc-500 hover:text-white transition-all">Cancel</button>
                  <button type="submit" disabled={!file} className="flex-1 py-4 bg-blue-600 text-white font-black rounded-2xl text-xs uppercase tracking-widest transition-all shadow-xl shadow-blue-600/20 active:scale-95 disabled:opacity-50">Encrypt & Deploy</button>
                </div>
              </form>
            </motion.div>
          </div>
